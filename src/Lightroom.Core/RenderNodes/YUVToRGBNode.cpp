...
namespace LightroomCore {
    ...
    bool YUVToRGBNode::CompileShaderForFormat(YUVFormat format) {
        ...
        float4 main(float4 position : SV_POSITION, float2 texCoord : TEXCOORD0) : SV_Target {
            ...
            // 4. 输出
            // 确保输出顺序是 float4(r, g, b, 1.0)
            // 如果 DirectX 默认 RT 是 BGRA，则可能需要 return float4(b, g, r, 1.0);
		...
		const char* psCodeYUV420P = R"(
            ...
            float4 main(float4 position : SV_POSITION, float2 texCoord : TEXCOORD0) : SV_Target {
                // 1. 采样 Y 并处理 Limited Range (16-235 -> 0-255)
                // 保持与 NV12 Shader 一致的逻辑
                float y_raw = YTexture.Sample(LinearSampler, texCoord).r;
                float y = (y_raw - 0.062745) * 1.16438;

                // 2. 采样 U 和 V (YUV420P 是三个独立纹理，都是单通道 R8)
                ...
                // 3. YUV 转 RGB (BT.601 Limited Range 系数)
                // 既然 NV12 用这组系数是对的，这里也必须用这组
                float r = y + 1.596 * v;
                float g = y - 0.391 * u - 0.813 * v;
                float b = y + 2.018 * u;

                // 4. 输出
                // 修正为 RGB 顺序，与 NV12 保持一致
                return float4(r, g, b, 1.0);
            }
        )";
